...
export class SettingsDocument {
    ...
    public async provideCompletionItems(
        ...
        const allConfig = await this.getAllDependencies(this.document.getText());
        ...
        if (currDependencyName.length) {
            ...
            return await this.getMatchedVersions(currDependencyName, allConfig);
        }
    ...
    private async getAllDependencies(configText: string) {
            ...
            if (start) {
                if (line.search(']') !== -1) {
                    ...
                    if (single) {
                        ...
                        let end = line.indexOf('"', start + 1);
                        let name = line.slice(start + 1, end);
                        if (name.length) {
        ...
        for (let single in configs) {
            let hash = await this.verMgr.getPortVersionHash(configs[single].name, configs[single].version);
            if (hash.length) {
    ...
    private async getMatchedVersions(name: string, allConfig: Array<{ name: string; version: string; hash: string }>) {
        let versions = await this.verMgr.getPortVersions(name);
